...
本文件是 **99机场吧（99jc.xyz）** 的长期项目开发规范，供 Claude Code 在本项目中进行任何开发工作时遵循。修改本项目代码前应先阅读并遵循本文件。
...
- **中文品牌**：99机场吧
- **英文品牌 / 项目名称**：99Jichangba
- **正式域名**：https://99jc.xyz

### 品牌使用规则

"99机场吧"是面向中文用户的正式品牌名称，"99Jichangba"是英文品牌/项目名称。

所有面向用户的页面（Header、Footer、Title、Meta Description、Open Graph、JSON-LD、Breadcrumb、About、首页、文章页等）**优先使用"99机场吧"**。

**禁止出现**以下未经确认的品牌写法：`99JiChangBa`、`Jichang99吧`、`99机场吧99Jichangba`（中英重复堆叠），或其他中英混写变体。

英文品牌 `99Jichangba` 只在确实需要英文品牌名、代码变量、项目名称等场景使用（如 `package.json` 的 `name` 字段、代码仓库名）。
...
### 姊妹站网络——同一站长旗下四个站点，禁止内容雷同

本站（99机场吧 / 99jc.xyz）是同一站长名下站群里的第 4 个站点。截至本站建设时，站群内其他站点情况如下：

| 站点 | 域名 | 定位角度 |
| --- | --- | --- |
| 机场吧 | jc8.mom | 导航 + 知识科普综合入口，中性、克制的信息站语气 |
| 机场荟 | 8jc.mom | 横向对比 + 选购决策，同一套维度并排比较不同服务商 |
| 99机场梯子导航 | 99jc.my | 站群第 3 个站点，与本站同批并行建设，建设时可能尚未上线 |
| 爱机场推荐 | ajc.lol | 站群第 5 个站点，与本站同批并行建设，建设时可能尚未上线 |

这些站点和 jichangcha.co / itizi.mom 性质不同——不是外部竞品，是自己人，但同一站长名下的多个域名互相抄袭/内容高度雷同，对每一个域名的 SEO 都是减分，因此规则和对待外部竞品同样严格，甚至更严格：

- **正文文案禁止跨站复用**：哪怕是同一个 Provider 的同一批客观事实（价格、协议、线路），99机场吧这边的描述、编辑结论、行文角度都必须重新组织表达，不能把任何姊妹站的句子换个域名贴过来。
- **品牌视觉必须独立**：色彩方案、Logo 图形、页面装饰风格都要和所有姊妹站区分开，不能一眼看出"这几个是同一个模板换皮"。
- **网站定位需要差异化**：机场吧是"导航+知识科普"综合入口；机场荟是"横向对比+选购决策"；99机场吧走的是"老司机唠嗑/社区闲聊"角度——过来人经验分享、FAQ/问答式的信息组织，语气更随性、更像论坛老哥在给你讲人话，而不是横向对比表或知识库条目。首页、栏目权重、内容侧重、行文语气都应该体现这个差异，不是简单复制一份栏目结构或者换一套模板皮肤。
- **Provider 排行榜的"共享锚点"不算雷同**：综合榜前 7 名沿用站群里已经验证过的顺序（无忧链接固定首位），这代表站长对核心合作品牌的一贯排序，是站群共享的事实性锚点，不属于"雷同"问题；**第 8 名及以后由本站独立设计排序与收录范围**。但排序背后的表达方式（编辑理由怎么写、样式怎么呈现）仍必须独立原创，不能直接照抄其他站的措辞或视觉。
- **关于站名相似性的特别提醒**：本站中文品牌"99机场吧"和机场吧（jc8.mom）的"机场吧"文字上非常接近（只多了"99"前缀），这种情况下更容易被读者、搜索引擎甚至站长自己混淆成"同一个东西的马甲"。**正因为名字像，定位、语气、视觉三件事必须做得比其他姊妹站之间的区分度更大，不能因为"反正名字都不一样"就偷懒**——本站选择"社区闲聊/经验分享"这个角度、靛蓝+青的配色、聊天气泡 Logo，都是为了在名字相似的前提下，让人一眼能分清这是两个不同性格的产品，而不是同一个站换了皮。
- 站群内各站点可以互相设置友情链接（这是正常的站群操作，参考 itizi.mom / maitizi.com 页脚的友情链接做法），但内容本身必须各自独立原创。

---

## 27. 最终原则

99Jichangba 不是一个"AI 批量生成文章的 SEO 网站"。
