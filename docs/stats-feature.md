...
1. 创建 KV Namespace：Cloudflare Dashboard → Workers & Pages → KV → Create a namespace，名字随意（建议 `99jcxyz-stats`）。
2. 绑定到 Pages 项目：进入 99jc.xyz 对应的 Pages 项目 → Settings → Functions → KV namespace bindings → 新增一条，**变量名必须填 `STATS_KV`**（代码里硬编码了这个名字），选择刚创建的 namespace。**Production 和 Preview 环境要分别绑定一次**，只绑其中一个的话另一个环境会一直报 500。
